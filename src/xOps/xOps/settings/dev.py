...
SECRET_KEY = '27)pzie0tu__c7a=aw2_i#w$r&s$=grot)(b5qe$b&5c)sadlh'
...
DATABASES = {
    'default': {
        'ENGINE': 'django.db.backends.mysql',
        'HOST': '39.105.100.168',  # 数据库主机
        'PORT': 3306,  # 数据库端口
        'USER': 'root',  # 数据库用户名
        'PASSWORD': '123456',  # 数据库用户密码
        'NAME': 'xops_rest'  # 数据库名字
    }
}
...
LOGGING = {
    ...
    'handlers': {  # 日志处理方法
        ...
        'file': {  # 向文件中输出日志
            'level': 'INFO',
            'class': 'logging.handlers.RotatingFileHandler',
            'filename': os.path.join(os.path.dirname(BASE_DIR), "logs/xOps.log"),  # 日志文件的位置
            'maxBytes': 300 * 1024 * 1024,
            'backupCount': 10,
            'formatter': 'verbose'
        },
...
EST_FRAMEWORK = {
    # 指定DRF框架异常处理函数
    'EXCEPTION_HANDLER': 'xOps.utils.exceptions.exception_handler',
}
